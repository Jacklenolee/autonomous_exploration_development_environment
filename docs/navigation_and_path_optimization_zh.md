...
需要注意，A* 只在它构建出的二维占据栅格上搜索。RViz 中点得很远时，如果目标已经落到 preview 点云覆盖范围之外，或者起点到目标之间的地面点云太稀疏导致自由区域断开，那么 A* 会认为起点和终点不连通。当前版本为这种情况增加了 `shortestPathUseRelaxedGroundFallback`：严格地面连通搜索失败后，会再尝试一张“放宽地面连通、仍保留障碍物膨胀”的兜底栅格。因此远距离 waypoint 更容易显示参考最短路径，但已识别的墙体、树木、木板等障碍仍然会阻挡 A*。
...
远距离 waypoint 不显示，最常见的原因有四类：

1. 目标点超出 preview 点云生成的 A* 栅格范围。
2. 目标点附近没有可通行自由格，最近自由格搜索半径 `shortestPathNearestFreeRadius` 内找不到可落脚位置。
3. 起点和终点都在地图内，但中间地面点云过稀疏，严格地面栅格被分割成多个不连通区域。
4. 障碍物膨胀或动态障碍膨胀把窄通道封闭，导致 A* 没有可走通道。

这里的“地面点云过稀疏”很关键。严格栅格不是把整张地图都当作可走区域，而是先找地面点，再对地面做有限膨胀。这样能避免路径穿过未知区域，但也会带来一个副作用：当远处 preview 点云采样不连续时，中间可能出现没有地面点的空洞，A* 就会断开。

当前版本增加了 `shortestPathUseRelaxedGroundFallback`：

```text
shortestPathUseRelaxedGroundFallback = true
```

它的处理顺序是：

1. 先用严格栅格搜索。严格栅格要求地面点云连通，并叠加静态障碍、动态障碍和安全膨胀。
2. 如果严格栅格失败，先尝试去掉动态障碍层，只用静态 preview 地图再算一次。
3. 如果仍然失败，再构建宽松兜底栅格：默认把非障碍区域当作可通行，只把静态障碍和动态障碍膨胀后的区域标为不可通行。
4. 如果宽松兜底成功，则继续发布 `/shortest_path`，并在终端输出 warning，说明严格地面连通失败但兜底成功。
5. 如果兜底仍然失败，则保留上一条有效最短路径，并输出失败原因。

这个兜底不是把最短路径改成无障碍直线。它只是放宽“未知地面是否连通”的判断，障碍物仍然来自：

- preview 点云中的静态障碍
- `/registered_scan` 中的动态障碍
- `shortestPathObstacleInflation`
- `shortestPathDynamicObstacleInflation`
- line-of-sight 安全半径检查

因此它适合解决“点远了、地面采样断开、但中间没有明确障碍”的不显示问题；如果目标真的在墙后、障碍包围区、地图外，A* 仍然会失败。
...
3. 如果静态地图仍然因为地面连通不足而失败，则尝试 `shortestPathUseRelaxedGroundFallback` 宽松地面兜底。
4. 如果任一级能找到路，继续发布 `CurrentShortestPath`，并在终端输出 warning，说明触发了哪一级 fallback。
5. 如果所有策略都失败，则保留上一条有效最短路径，并输出具体失败原因，例如目标超出地图、起终点附近没有自由格、A* 无法连通等。

这样处理的目的不是忽略障碍物，而是避免瞬时激光噪声、临时膨胀或远处地面点云稀疏把评估用最短路径完全清空。真正用于评估的 `L_shortest` 仍然来自障碍物约束 A*；当动态层可靠时优先使用动态层，当动态层或严格地面连通导致不可达时才进入 fallback。
...
消息类型为 `visualization_msgs/msg/MarkerArray`，RViz 中显示名称为 `ShortestPath`。内部使用 `Marker::LINE_STRIP`。每次点击 waypoint 并成功规划后，系统会把这一整段理论最短路径追加到红色历史轨迹；下一次 waypoint 成功规划后，再把下一整段追加进去。这样红色 `ShortestPath` 表示“每次任务级规划得到的理论最短路径轨迹”，可以和实际 `Trajectory` 做整体对比。
...
注意，红色历史线不会在车辆每次 odometry 更新时反复追加局部前缀。它只在新的 waypoint 成功规划时追加整段最短路径。这样做是为了避免短前缀反复叠加导致轨迹又断又乱，并保持和本系统实际 `/path`、`/trajectory` 的任务级记录思路一致：

```text
waypoint 1 -> shortest path segment 1
waypoint 2 -> shortest path segment 2
waypoint 3 -> shortest path segment 3
```
